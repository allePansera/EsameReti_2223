...
def parse_request(conn):
    """
    La funzione ritorna lo URL richiesto
    """
    request = ''
    while True:
        # conn.recv ritorna un array, quindi devo decodificarlo
        request += conn.recv(1024).decode('utf-8')

        # controllo la dimensione della risposta
        if request.find('\r\n\r\n') > 0:
            break
...
def send_200(conn, fname):
    ...
    with open(fname, 'rb') as f:
        l = f.read(1024)  # ritorna 0 se il file è vuoto
        while l:
            conn.send(l)
            l = f.read(1024)
    time.sleep(1)
